...
class Conta():
    ...
    def deposita(self, valor, transferencia=False):
        ...
        self.saldo += valor
        if transferencia:
            self.historico.transacoes.append("{} - Transferência recebida - valor: {}".format(datetime.today(), valor))
            return True, "Transferência realizada com sucesso"
        else:
            self.historico.transacoes.append("{} - Depósito - valor: {}".format(datetime.today(), valor))
    ...
    def saca(self, valor, transferencia=False):
        ...
        self.saldo -= valor
        if transferencia:
            self.historico.transacoes.append("{} - Transferência realizada - valor: {}".format(datetime.today(), valor))
            return True, "Transferência realizada com sucesso"
        else:
            self.historico.transacoes.append("{} - Saque - valor: {}".format(datetime.today(), valor))
            return True, "Saque realizado com sucesso"
    
    def extrato(self):
        self.historico.imprime()
        print("- {} - Saldo de {} do cliente {}".format(datetime.today(), self.saldo, self.cliente.nome))
        print("-" * 60)
